-- Naprawa funkcji symulacji - supervisor_id może być NULL (FK do auth.users)
...
BEGIN
    ...
    VALUES (v_company_id, v_supplier_id, 'Received', 'Paleta EUR', 15, 'PZ/2026/01/001'), (v_company_id, v_supplier_id, 'Received', 'E2', 100, 'PZ/2026/01/001');
    
    -- MES: Decomposition (bez supervisor_id - FK do auth.users)
    ...
    WITH numbered_logs AS (
        ...
        FROM public.t_production_logs WHERE production_order_id = v_order_kebab_id AND product_id = v_product_kebab_id
    )
    UPDATE public.t_production_logs pl
    SET handling_unit_id = CASE WHEN nl.rn <= 55 THEN v_pallet_1_id WHEN nl.rn <= 110 THEN v_pallet_2_id WHEN nl.rn <= 165 THEN v_pallet_3_id ELSE v_pallet_4_id END
    ...
    RETURN jsonb_build_object(
        ...
        'summary', jsonb_build_object(
            ...
            'input_weight_kg', 5000, 'fillet_produced_kg', 3000, 'bones_produced_kg', 1900,
            'technological_loss_kg', 100, 'yield_decomposition_pct', 60,
            'kebab_produced_kg', 3300, 'kebab_blocks', 220, 'pallets_created', 4,
            'shipment_status', 'Shipped', 'bones_remaining_kg', 1900, 'kebab_remaining_kg', 0,
            'chiller_location_id', v_location_chiller, 'freezer_location_id', v_location_freezer,
            'batches_with_location', 4, 'packaging_received', 2, 'packaging_issued', 2
        ),
        'employees', jsonb_build_object('jan_kowalski_id', v_employee_jan, 'adam_nowak_id', v_employee_adam, 'anna_zmiana_id', v_employee_anna),
