...
BEGIN
  ...
  INSERT INTO t_warehouse_movements (company_id,facility_id,document_number,type,contractor_id,received_temp_c,received_temp_method,status)
    VALUES (v_company_id,v_facility_id,v_pz_number,'PZ',v_supplier_id,p_received_temp_c,'MANUAL_PROBE','Approved') RETURNING id INTO v_pz_id;
  ...
  INSERT INTO t_batches (product_id,internal_batch_number,parent_batch_id,source_event_type,initial_quantity,current_quantity,status,location_id,production_date,expiration_date)
    VALUES (v_p_kebab,to_char(now(),'YYMMDDHH24MISS')||'/AUDIT/KEB',v_meat_batch_id,'ASSEMBLY',700,700,'Released',v_loc_id,current_date,(now()+interval '180 days')::date) RETURNING id INTO v_kebab_batch_id;
  ...
  INSERT INTO t_production_logs (production_order_id,source_batch_id,output_batch_id,product_id,weight_gross,weight_tare,process_stage,packaging_type,packaging_count,recipe_id)
    VALUES (v_assembly_id,v_meat_batch_id,v_kebab_batch_id,v_p_kebab,700,0,'Stacking','Słupek 5kg',140,v_recipe_id);
  ...
  INSERT INTO t_production_logs (production_order_id,source_batch_id,output_batch_id,product_id,weight_gross,weight_tare,process_stage,freezing_started_at,freezing_completed_at,freezing_duration_minutes,latest_core_temp_c,target_core_temp_c,ccp_passed)
    VALUES (v_freeze_id,v_kebab_batch_id,v_kebab_batch_id,v_p_kebab,700,0,'ShockFreezing',now()-interval '4 hours',now(),240,-20.0,-18.0,true) RETURNING id INTO v_freeze_log_id;
  INSERT INTO t_freezing_temp_log (production_log_id,recorded_at,core_temp_c,ambient_temp_c,source) VALUES (v_freeze_log_id,now(),-20.0,-28.0,'manual');
  ...
  INSERT INTO t_shipments (company_id,facility_id,shipment_number,customer_id,status,dispatch_date,driver_name,truck_plates)
    VALUES (v_company_id,v_facility_id,v_shipment_number,v_customer_id,'Shipped',current_date,'Audyt Kierowca','WA AUDIT') RETURNING id INTO v_shipment_id;
  INSERT INTO t_shipment_items (shipment_id,handling_unit_id,product_id,quantity) VALUES (v_shipment_id,v_pallet_id,v_p_kebab,700);

  SELECT count(*) INTO v_lineage_count FROM t_lot_lineage WHERE child_lot_id IN (v_raw_batch_id,v_meat_batch_id,v_kebab_batch_id) OR child_handling_unit_id = v_pallet_id;
  SELECT count(*) INTO v_complaint_count FROM t_supplier_complaints WHERE movement_id = v_pz_id;

  RETURN jsonb_build_object('success',true,'company_id',v_company_id,'pz_id',v_pz_id,'pz_number',v_pz_number,'received_temp_c',p_received_temp_c,
    'raw_batch_id',v_raw_batch_id,'meat_batch_id',v_meat_batch_id,'kebab_batch_id',v_kebab_batch_id,
    'pallet_id',v_pallet_id,'sscc',v_sscc,'shipment_id',v_shipment_id,'shipment_number',v_shipment_number,
    'lineage_count',v_lineage_count,'complaint_count',v_complaint_count);
END; $$;