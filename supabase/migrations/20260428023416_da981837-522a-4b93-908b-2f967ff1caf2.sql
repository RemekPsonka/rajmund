...
BEGIN
  ...
  WITH RECURSIVE pallet_lots AS (
    -- partie bezpośrednio na palecie (z production_logs)
    SELECT DISTINCT COALESCE(pl.output_batch_id, pl.source_batch_id) AS lot_id
    FROM t_production_logs pl
    WHERE pl.handling_unit_id = NEW.id
      AND COALESCE(pl.output_batch_id, pl.source_batch_id) IS NOT NULL
  ),
  lineage_chain AS (
    SELECT pl.lot_id AS root_lot_id, pl.lot_id AS lot_id, 0 AS depth
    FROM pallet_lots pl
    UNION
    SELECT lc.root_lot_id, ll.parent_lot_id, lc.depth + 1
    FROM t_lot_lineage ll
    JOIN lineage_chain lc ON ll.child_lot_id = lc.lot_id
    WHERE lc.depth < 10 AND ll.parent_lot_id IS NOT NULL
  ),
  -- root jest "zmrożony" gdy w jego łańcuchu (sam batch lub jakikolwiek przodek)
  -- istnieje production_log etapu mrożenia z ccp_passed=true
  frozen_roots AS (
    SELECT DISTINCT lc.root_lot_id
    FROM lineage_chain lc
    WHERE EXISTS (
      SELECT 1 FROM t_production_logs plf
      WHERE plf.ccp_passed = true
        AND plf.process_stage IN ('ShockFreezing','Freezing')
        AND (plf.source_batch_id = lc.lot_id OR plf.output_batch_id = lc.lot_id)
    )
  )
  SELECT array_agg(DISTINCT b.internal_batch_number)
  INTO v_unfrozen_lots
  FROM pallet_lots pl
  JOIN t_batches b ON b.id = pl.lot_id
  WHERE pl.lot_id NOT IN (SELECT root_lot_id FROM frozen_roots);

  IF v_unfrozen_lots IS NOT NULL AND array_length(v_unfrozen_lots, 1) > 0 THEN
    RAISE EXCEPTION 'CCP3_FAILED: paleta zawiera partie bez zatwierdzonego mrożenia (CCP3 ≤ -18°C): %',
...
BEGIN
  ...
  VALUES
    (v_company_id, v_facility_id, v_pz_number, 'PZ', v_supplier_id, p_received_temp_c, 'MANUAL_PROBE', 'Approved')
  ...
  VALUES (v_p_raw, v_supplier_id, 'SUP-RAW-001',
          to_char(now(),'YYMMDDHH24MISS') || '/AUDIT/RAW',
  ...
  INSERT INTO t_production_orders (company_id, facility_id, order_number, type, status, production_date)
    VALUES (v_company_id, v_facility_id, 'ZP-DEC-AUDIT-' || to_char(now(),'HH24MISS'), 'Decomposition', 'Closed', current_date)
  ...
  INSERT INTO t_batches (product_id, internal_batch_number, parent_batch_id, source_event_type,
                         ...
                         expiration_date)
    VALUES (v_p_meat, to_char(now(),'YYMMDDHH24MISS')||'/AUDIT/MEAT', v_raw_batch_id, 'DISASSEMBLY',
  ...
  INSERT INTO t_lot_lineage (parent_lot_id, child_lot_id, event_type, qty_kg, process_ref_id, occurred_at)
    VALUES (v_raw_batch_id, v_meat_batch_id, 'DISASSEMBLY', 700, v_decomp_id, now());
  ...
  INSERT INTO t_production_orders (company_id, facility_id, order_number, type, status, production_date, recipe_id)
    VALUES (v_company_id, v_facility_id, 'ZP-ASM-AUDIT-' || to_char(now(),'HH24MISS'), 'Assembly', 'Closed', current_date, v_recipe_id)
  ...
  INSERT INTO t_batches (product_id, internal_batch_number, parent_batch_id, source_event_type,
                         initial_quantity, current_quantity, status, location_id, production_date,
                         expiration_date)
    VALUES (v_p_kebab, to_char(now(),'YYMMDDHH24MISS')||'/AUDIT/KEB', v_meat_batch_id, 'ASSEMBLY',
  ...
  INSERT INTO t_production_orders (company_id, facility_id, order_number, type, status, production_date)
    VALUES (v_company_id, v_facility_id, 'ZP-FRZ-AUDIT-' || to_char(now(),'HH24MISS'), 'Freezing', 'Closed', current_date)
  ...
  RETURN jsonb_build_object(
    'success', true,
    'company_id', v_company_id, 'facility_id', v_facility_id,
    'pz_id', v_pz_id, 'pz_number', v_pz_number, 'received_temp_c', p_received_temp_c,
