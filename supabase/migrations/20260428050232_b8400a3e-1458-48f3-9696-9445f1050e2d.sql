...
-- patch: poprawne wartości source_event_type
...
BEGIN
  ...
  INSERT INTO t_batches (product_id, internal_batch_number, parent_batch_id, source_event_type, initial_quantity, current_quantity, production_date, expiration_date, status, location_id)
    VALUES (v_semi_meat_id, TO_CHAR(NOW(),'YYMMDD') || '/KTF/MIESO-001', v_raw_batch_id,'DISASSEMBLY',3000,3000,NOW()::date,(NOW()+INTERVAL '7 days')::date,'Released',v_storage_location_id)
    RETURNING id INTO v_meat_batch_id;
  INSERT INTO t_batches (product_id, internal_batch_number, parent_batch_id, source_event_type, initial_quantity, current_quantity, production_date, expiration_date, status, location_id)
    VALUES (v_waste_bones_id, TO_CHAR(NOW(),'YYMMDD') || '/KTF/KOSCI-001', v_raw_batch_id,'DISASSEMBLY',1900,1900,NOW()::date,(NOW()+INTERVAL '3 days')::date,'Released',v_storage_location_id)
    RETURNING id INTO v_bones_batch_id;
  INSERT INTO t_batches (product_id, internal_batch_number, parent_batch_id, source_event_type, initial_quantity, current_quantity, production_date, expiration_date, status, location_id)
    VALUES (v_waste_skin_id, TO_CHAR(NOW(),'YYMMDD') || '/KTF/SKORA-001', v_raw_batch_id,'DISASSEMBLY',100,100,NOW()::date,(NOW()+INTERVAL '3 days')::date,'Released',v_storage_location_id)
  ...
  INSERT INTO t_lot_lineage (parent_lot_id, child_lot_id, event_type, qty_kg, process_ref_id, operator_id) VALUES
    (v_raw_batch_id, v_meat_batch_id,  'DISASSEMBLY', 3000, v_decomp_order_id, v_emp_dec),
    (v_raw_batch_id, v_bones_batch_id, 'DISASSEMBLY', 1900, v_decomp_order_id, v_emp_dec),
    (v_raw_batch_id, v_skin_batch_id,  'DISASSEMBLY', 100,  v_decomp_order_id, v_emp_dec);

  -- 3. Masowanie (TUMBLING)
  ...
  INSERT INTO t_batches (product_id, internal_batch_number, parent_batch_id, source_event_type, initial_quantity, current_quantity, production_date, expiration_date, status, location_id)
    VALUES (v_semi_masa_id, TO_CHAR(NOW(),'YYMMDD') || '/KTF/MASA-001', v_meat_batch_id,'TUMBLING',3300,3300,NOW()::date,(NOW()+INTERVAL '5 days')::date,'Released',v_storage_location_id)
  ...
  INSERT INTO t_lot_lineage (parent_lot_id, child_lot_id, event_type, qty_kg, process_ref_id, operator_id)
    VALUES (v_meat_batch_id, v_masa_batch_id, 'TUMBLING', 3300, v_processing_order_id, v_emp_prc);

  -- 4. Składanie (ASSEMBLY)
  INSERT INTO t_production_orders (company_id, facility_id, order_number, type, status, production_date, supervisor_id)
  ...
  INSERT INTO t_batches (product_id, internal_batch_number, parent_batch_id, source_event_type, initial_quantity, current_quantity, production_date, expiration_date, status, location_id)
    VALUES (v_finished_kebab_id, TO_CHAR(NOW(),'YYMMDD') || '/KTF/KEBAB-001', v_masa_batch_id,'ASSEMBLY',3300,3300,NOW()::date,(NOW()+INTERVAL '90 days')::date,'Released',v_storage_location_id)
  ...
  INSERT INTO t_lot_lineage (parent_lot_id, child_lot_id, event_type, qty_kg, process_ref_id, operator_id)
    VALUES (v_masa_batch_id, v_kebab_batch_id, 'ASSEMBLY', 3300, v_assembly_order_id, v_emp_asm);
  ...
  INSERT INTO t_production_inputs (production_order_id, batch_id, product_id, weight) VALUES (v_freezing_order_id, v_kebab_batch_id, v_finished_kebab_id, 3300);
  ...
  INSERT INTO t_production_logs (production_order_id, employee_id, source_batch_id, output_batch_id, product_id,
    ...
    VALUES (v_freezing_order_id, v_emp_asm, v_kebab_batch_id, v_kebab_batch_id, v_finished_kebab_id,
      3400, 100, 'ShockFreezing', NOW() - INTERVAL '4 hours', NOW(), 240, -22, -18, true)
  ...
  -- 6. Paletyzacja
  FOR v_i IN 1..5 LOOP
    INSERT INTO t_handling_units (company_id, facility_id, sscc_number, type, status, total_net_weight, total_gross_weight, items_count, production_date, label_printed)
      VALUES (v_company_id, v_facility_id, '0059' || LPAD(v_i::text, 16, '0'), 'Pallet','Open', 660, 685, v_kebab_per_pallet, NOW()::date, true)
    ...
    INSERT INTO t_production_logs (production_order_id, employee_id, source_batch_id, output_batch_id, product_id,
      ...
      latest_core_temp_c, target_core_temp_c, ccp_passed)
      VALUES (v_freezing_order_id, v_emp_asm, v_kebab_batch_id, v_kebab_batch_id, v_finished_kebab_id,
        v_pallet_id, 685, 25, 'ShockFreezing', NOW() - INTERVAL '4 hours', NOW(), 240, -22, -18, true);
  ...
  INSERT INTO t_shipments (company_id, facility_id, shipment_number, status, dispatch_date,
    customer_id, driver_name, truck_plates, trailer_plates, seal_number, transport_temperature, total_net_weight, pallets_count)
    VALUES (v_company_id, v_facility_id, v_shipment_number, 'Shipped', NOW()::date,
      v_customer_id, 'Jan Kierowca', 'WA 12345', 'WA 99999X', 'PL-SEAL-2026-001', -20, 3300, 5)
  ...
  v_result := jsonb_build_object(
    ...
    'summary', jsonb_build_object(
      'raw_input_kg', 5000,
      'receiving', jsonb_build_object('document_number', v_pz_number, 'supplier_name','Ferma Drobiu ABC','temp_c',v_pz_temp,'ccp1_passed',(v_pz_temp <= 4)),
      'decomposition', jsonb_build_object('meat_kg',3000,'bones_kg',1900,'skin_kg',100,'yield_percent',round(3000.0/5000*100,1)),
      'processing', jsonb_build_object('input_kg',3000,'output_kg',3300,'yield_percent',round(3300.0/3000*100,1)),
      'assembly', jsonb_build_object('kebab_10kg_count',30,'kebab_15kg_count',100,'kebab_20kg_count',75,'total_kebabs',205,'total_weight_kg',3300),
      'freezing', jsonb_build_object('items_frozen',205,'duration_hours',4,'temperature_celsius',-22,'ccp_passed',true),
      'logistics', jsonb_build_object('pallets_created',5,'shipment_status','Shipped','shipment_number',v_shipment_number)
    ),
    'products_by_category', jsonb_build_object('RawMeat',1,'SemiFinished',2,'FinishedGood',1,'Waste',2,'Spice',1),
    'orders_by_type', jsonb_build_object('Decomposition',1,'Processing',1,'Assembly',1,'Freezing',1),
    'recipe', jsonb_build_object('name','Receptura Masa Kebabowa','base_product','Mięso z kurczaka (po rozbiorze)','base_category','SemiFinished','output_product','Masa kebabowa masowana','target_yield_percent',110),
    ...
    'traceability', jsonb_build_object(
      'delivery_id',v_pz_id,'raw_batch_id',v_raw_batch_id,'meat_batch_id',v_meat_batch_id,
      'masa_batch_id',v_masa_batch_id,'kebab_batch_id',v_kebab_batch_id,'shipment_id',v_shipment_id,
