-- 1) Fix audit_e2e_flow: use generate_sscc_number(company_id) instead of generate_sscc_mod10()
...
-- 2) Extend cleanup_demo_data with movements + complaints
...
BEGIN
  ...
  DELETE FROM t_freezing_temp_log WHERE production_log_id IN (
  ...
  DELETE FROM t_lot_lineage
   WHERE child_lot_id IN (SELECT b.id FROM t_batches b JOIN t_products p ON p.id=b.product_id WHERE p.company_id=v_company_id)
      OR parent_lot_id IN (SELECT b.id FROM t_batches b JOIN t_products p ON p.id=b.product_id WHERE p.company_id=v_company_id)
      OR child_handling_unit_id IN (SELECT id FROM t_handling_units WHERE company_id=v_company_id);

  DELETE FROM t_production_kebab_variants WHERE production_log_id IN (
    SELECT pl.id FROM t_production_logs pl
    JOIN t_production_orders po ON po.id = pl.production_order_id WHERE po.company_id = v_company_id
  );
