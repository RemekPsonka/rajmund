-- Aktualizacja funkcji symulacji Golden Path z pełnym wsparciem dla lokalizacji, opakowań i kierunków
...
BEGIN
    ...
    -- Create Storage Locations for the facility
    ...
    -- Create input batch (5000 kg raw material) with location
    ...
    -- Record packaging received from driver (PZ)
    INSERT INTO public.t_packaging_transactions (company_id, contractor_id, type, packaging_type, quantity, comments)
    VALUES 
        (v_company_id, v_supplier_id, 'Received', 'Paleta EUR', 15, 'PZ/2026/01/001 - zwrot palet'),
        (v_company_id, v_supplier_id, 'Received', 'E2', 100, 'PZ/2026/01/001 - zwrot pojemników');
    ...
    -- Issue raw material (RW) - 5000 kg with direction SWIEZE
    ...
    -- Create fillet batch (output) with location
    INSERT INTO public.t_batches (
    ...
    -- Create bones batch in stock with location
    INSERT INTO public.t_batches (
        product_id, internal_batch_number, initial_quantity, current_quantity,
        production_date, expiration_date, status, location_id
    )
    ...
    -- Issue fillet (RW) - 3000 kg with direction KEBAB
    INSERT INTO public.t_production_inputs (production_order_id, batch_id, product_id, weight, created_by, direction)
    ...
    -- Create kebab batch (output with mass increase from injection - 3300 kg from 3000 kg input) with location
    INSERT INTO public.t_batches (
        product_id, internal_batch_number, initial_quantity, current_quantity,
        production_date, expiration_date, status, location_id
    )
    ...
    -- Add packaging transactions (E2 containers and EUR pallets) - Issued to customer
